...
def CreatePivotTable(inTable, outTable):
    ...
    arcpy.Statistics_analysis(inTable, outTable, "refcode COUNT;dm_stat COUNT", "refcode;dm_stat")

    arcpy.PivotTable_management(outTable, 'refcode', 'dm_stat', 'FREQUENCY', os.path.join(env.workspace, "pivotTable"))
...
def dmbiologist(pivotTable):
    ...
    for biologist in biologists:
        if biologist is None:
            ...
            tableTEMP = arcpy.TableToTable_conversion(pivotTableCOPY, "in_memory", "tableTEMP", expression)
...
##reportType = input("Which report would you like to run? (please enter 'DM Pending', 'DM Ready', 'DM Biologist', 'DM Reviewer', 'DM Total, or DM All')")
reportType = "dm all"
