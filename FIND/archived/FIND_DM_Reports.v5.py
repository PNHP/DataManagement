...
for i, output in zip(input_features, elementShapefiles):
    ...
    spatial_join = arcpy.SpatialJoin_analysis(target_features, join_features, element_features)


'''function that assigns element type to all features based on name of shapefile within which the feature is contained'''
...
for elementShapefile in elementShapefiles:
    ...
    arcpy.CalculateField_management(input_feature, "element_type", "'" + elementShapefile + "'", "PYTHON_9.3")
...
elementRecords = os.path.join(env.workspace, "elementRecords")
arcpy.Merge_management(elementTables, elementRecords)
...
ETtableEXCEL = r'P:\\Conservation Programs\\Natural Heritage Program\\Data Management\\Instructions, procedures and documentation\\FIND\\FIND_2017\\DM Documentation\\Admin and Maintenance\\20170321_ET.csv'
arcpy.TableToTable_conversion (ETtableEXCEL, "memory", "ETtable")
arcpy.JoinField_management(elementRecords, "elem_name", "memory\\ETtable", "ELEMENT_SUBNATIONAL_ID", ["ELEMENT_CODE", "SCIENTIFIC_NAME"])

arcpy.AddField_management(elementRecords, "Location", "TEXT", "", "", 1, "Location", "", "", "")
...
arcpy.AddField_management(elementRecords, "Reviewer", "TEXT", "", "", 35, "ID Reviewer", "", "", "")

with arcpy.da.UpdateCursor(elementRecords, ['ELEMENT_CODE', 'Location', 'Reviewer']) as cursor:
    for row in cursor:
...
keepFields = ["OID", "OBJECTID", "COUNTY_NAM", "refcode", "created_user", "created_date",
...
outPath = "P:\\Conservation Programs\\Natural Heritage Program\\Data Management\\Instructions, procedures and documentation\\FIND\\FIND_2021\\Reports\\ID Reviewers Status Reports"
...
for reviewer in reviewers:
    if reviewer is None:
        ...
        tableTEMP = arcpy.TableToTable_conversion(elementRecords, "memory", "tableTEMP", expression)
...
arcpy.Statistics_analysis(elementRecords, outTable, "refcode COUNT;dm_stat COUNT", "refcode;dm_stat")

pivotTable = arcpy.PivotTable_management(outTable, 'refcode', 'dm_stat', 'FREQUENCY', os.path.join(env.workspace, "pivotTable"))
...
arcpy.AddField_management(pivotTable, "total_records", "DOUBLE", field_length = 3, field_alias = "Total Records")
expression = "!dmpend! + !dmproc! + !dmready! + !idprob! + !dr! + !idrev!"
arcpy.CalculateField_management(pivotTable, "total_records", expression, "PYTHON_9.3")

# join dm status and dm status comments data from survey site to pivot table
join = os.path.join(env.workspace, "survey_site1")
arcpy.AlterField_management(join, "dm_stat", "survey_site_dmstat", "Survey Site - DM Status")
arcpy.JoinField_management(pivotTable, "refcode", join, "refcode", ["survey_site_dmstat", "dm_stat_comm", "elem_found"])

# join original data from elementRecords table to pivot table
join = os.path.join(env.workspace, "elementRecords")
arcpy.JoinField_management(pivotTable, "refcode", join, "refcode", ["COUNTY_NAM", "created_user", "created_date", "last_up_by", "last_up_on", "element_type"])

# add new field for east or west location
arcpy.AddField_management(pivotTable, "Location", "TEXT", "", "", 1, "Location", "", "", "")
...
outPath = "P:\\Conservation Programs\\Natural Heritage Program\\Data Management\\Instructions, procedures and documentation\\FIND\\FIND_2021\\Reports\\Biologist Status Reports"
...
for biologist in biologists:
    if biologist is None:
        ...
        tableTEMP = arcpy.TableToTable_conversion(pivotTableCOPY, "memory", "tableTEMP", expression)
