...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [NegativeDataEntryTool,PermissionDeniedTool]
...
##        edit = arcpy.da.Editor(elem_poly)
##        edit.startEditing()
##        edit.startOperation()
        ...
        fields = ["refcode","elem_type","elem_name","elem_found","elem_found_comm","eoid","date_start","date_stop","dm_stat","feat_meth","feat_meth_comm","SHAPE@"]
...
##        params = [potential_site,ref_code,target_elements,last_name,first_name,position,instit]
    ...
    def execute(self, params, messages):
        ...
        if target_elements:
            arcpy.AddMessage(target_elements)
            elems = []
            for element in target_elements:
                ...
                fields = ["refcode","elem_type","elem_name","elem_found","elem_found_comm","eoid","dm_stat","feat_meth","feat_meth_comm","SHAPE@"]
                with arcpy.da.InsertCursor(elem_poly,fields) as cursor:
                    cursor.insertRow(values)

                # insert new negative record into nf species list
                values = [site_name,elem_type,"nc",elem_name]
                fields = ["prop_surv_site_n","target_type","survey_status"]
                with arcpy.da.InsertCursor(species_table,fields) as cursor:
                    cursor.insertRow(values)
                ...
                # add status message for user
            arcpy.AddMessage(len(target_elements)+" needs found species list record(s) and needs found element polygon(s) were added to the proposed survey "+site_name+" for the element(s): "+elems)
