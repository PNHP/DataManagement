...
for name in f_name:
    arcpy.AddField_management(cpp_eo_ptreps,f,"TEXT","","",25)
...
final_dict = {}
with arcpy.da.SearchCursor(cpp_eo_ptreps,['EO_ID',"core","supporting",'SNAME','SCOMNAME','specid','specid_2','specid_3','specid_comments','ER_RULE','EO_TRACK','ELSUBID','EO_TYPE']) as cursor:
    for row in cursor:
...
with open(os.path.join(reportPath, 'CPP_Outstanding_'+date+'.csv'), 'wb') as csvfile:
    ...
    csv_output.writerow(['EO_ID','Core Status','Supporting Status','SNAME','SCOMNAME','specid','specid_2','specid_3','specid_comments','ER_RULE','EO_TRACK','ELSUBID','EO_TYPE'])
