...
keepFields = [oid_field, 'Shape', 'EO_ID', 'SNAME', 'SCOMNAME', 'ELSUBID', 'EO_TYPE', 'EO_TRACK', 'ER_RULE', 'EO_RULE',
              'specid', 'specid_2', 'specid_3', 'specid_comments']
...
cursor = conn.cursor()
cursor.execute(
    "SELECT EOID, [Date created, updated or deleted], [Mapper - new, update, or deletion], [Brief description], County FROM [EO log] WHERE EOID IS NOT NULL AND [Mapper - new, update, or deletion] <> 'NA' AND [Date created, updated or deleted] >= #{0}# AND [Date created, updated or deleted] <= #{1}#".format(
        date1, date2))
...
with arcpy.da.UpdateCursor(cpp_eo_ptreps,
                           ["EO_ID", "date_created", "mapper_status", "description", "county"]) as cursor:
    for row in cursor:
